...
export default function MarketResearchListPage() {
  ...
  const fetchOrders = async () => {
    try {
      ...
      if (filter.assigned === 'mine' && user) {
        filteredData = filteredData.filter((order) => order.assigned_staff === user.id);
      }
  ...
  const handleViewDetails = (reservationNumber: string) => {
    router.push(`/staff/market-research/${reservationNumber}`);
  };

  return (
    <PageContainer
      title={isChineseStaff ? '市场调查管理' : '시장조사 관리'}
      description="시장조사 주문 관리 페이지"
    >
      <BlankCard>
        <CardContent>
          {/* 필터 섹션 */}
              ...
              <Table>
                <TableHead>
                  <TableRow>
                    ...
                    <TableCell>{isChineseStaff ? '目标价格' : '목표가'}</TableCell>
