...
export default function MarketResearchDetailPage() {
  ...
  const calculateValues = (data: any) => {
    ...
    if (updated.total_boxes && updated.box_length && updated.box_width && updated.box_height) {
      updated.total_cbm = (updated.total_boxes * updated.box_length * updated.box_width * updated.box_height) / 1000000;
    }
    ...
    if (updated.china_unit_price && updated.quoted_quantity && updated.exchange_rate) {
      ...
      updated.commission_amount = updated.china_unit_price * updated.quoted_quantity * updated.exchange_rate * 0.05;
    }

    // EXW 합계
    if (updated.china_unit_price && updated.quoted_quantity && updated.exchange_rate) {
      const chinaShippingInKrw = (updated.china_shipping_fee || 0) * updated.exchange_rate;
      updated.exw_total = (updated.china_unit_price * updated.quoted_quantity) * updated.exchange_rate + chinaShippingInKrw;
    }
    ...
    // 2차 결제비용
    if (shipping_fee > 0) {
      updated.expected_second_payment = shipping_fee + (updated.customs_duty || 0) + (updated.import_vat || 0);
    }

    // 예상 총 합계
    if (updated.first_payment_amount && updated.expected_second_payment) {
      updated.expected_total_supply_price = updated.first_payment_amount + updated.expected_second_payment;
    }
    ...
    try {
      ...
      if (!tariffResponse.error && tariffResponse.data?.success) {
        ...
        if (tariffData.fta_china?.rate !== undefined && tariffData.fta_china.rate < lowestRate) {
          ...
          rateType = '한미FTA';
        } else if (tariffData.fta_vietnam?.rate !== undefined && tariffData.fta_vietnam.rate < lowestRate) {
      ...
      if (!customsResponse.error && customsResponse.data?.success) {
        ...
        const certifications = customsResponse.data.requirements?.map((req: any) => 
          req.documentName || req.lawName
        ).filter(Boolean).join(', ');
              ...
              <TableBody>
                ...
                <TableRow>
                  ...
                  <TableCell>
                    ...
                    {data.moq_check ? (isChineseStaff ? '需要' : '필요') : (isChineseStaff ? '不需要' : '불필요')}
                  </TableCell>
                </TableRow>
                <TableRow>
                  ...
                  <TableCell>
                    ...
                    {data.logo_required ? (isChineseStaff ? '需要' : '필요') : (isChineseStaff ? '不需要' : '불필요')}
                  </TableCell>
                </TableRow>
                {data.logo_details && (
                  <TableRow>
                ...
                <TableRow>
                  ...
                  <TableCell>
                    ...
                    {data.custom_box_required ? (isChineseStaff ? '需要' : '필요') : (isChineseStaff ? '不需要' : '불필요')}
                  </TableCell>
                </TableRow>
                {data.box_details && (
                  <TableRow>
            ...
            <Grid container spacing={2}>
              ...
              <Grid size={{ xs: 12, md: 6 }}>
                <TextField
                  ...
                  onChange={(e) => editMode && setEditData({...editData, legal_type_kr: e.target.value})}
                  disabled={!editMode}
                  size="small"
                >
              ...
              <Grid size={{ xs: 12, md: 6 }}>
                <TextField
                  ...
                  value={editMode ? editData.company_size_kr || '' : data.company_size_kr || ''}
                  onChange={(e) => editMode && setEditData({...editData, company_size_kr: e.target.value})}
                  disabled={!editMode}
                  size="small"
              ...
              <Grid size={{ xs: 12, md: 6 }}>
                <TextField
                  ...
                  value={editMode ? editData.established_date || '' : data.established_date || ''}
                  onChange={(e) => editMode && setEditData({...editData, established_date: e.target.value})}
                  disabled={!editMode}
                  size="small"
              ...
              <Grid size={{ xs: 12, md: 6 }}>
                <TextField
                  ...
                  value={editMode ? editData.registered_capital || '' : data.registered_capital || ''}
                  onChange={(e) => editMode && setEditData({...editData, registered_capital: e.target.value})}
                  disabled={!editMode}
                  size="small"
              ...
              <Grid size={{ xs: 12, md: 6 }}>
                <TextField
                  ...
                  value={editMode ? editData.real_paid_capital || '' : data.real_paid_capital || ''}
                  onChange={(e) => editMode && setEditData({...editData, real_paid_capital: e.target.value})}
                  disabled={!editMode}
                  size="small"
                  placeholder="예: 100만 위안"
                />
                ...
                <FormControlLabel
                  control={
                    <Checkbox
                      ...
                      onChange={(e) => editMode && setEditData({...editData, is_small_business: e.target.checked})}
                      disabled={!editMode}
                    />
                  }
              ...
              {(isChineseStaff || userProfile?.role === 'korean_team' || userProfile?.role === 'admin') && (
                <>
                  <Grid size={{ xs: 12, md: 6 }}>
                    <TextField
                      fullWidth
                  ...
                  <Grid size={12}>
                    <TextField
                      ...
                      value={editMode ? editData.product_site_url || '' : data.product_site_url || ''}
                      onChange={(e) => editMode && setEditData({...editData, product_site_url: e.target.value})}
                      disabled={!editMode}
                      size="small"
                      helperText={isChineseStaff ? '1688或淘宝链接' : '1688 또는 타오바오 링크'}
                    />
                  </Grid>
                </>
              )}
            ...
            <Grid container spacing={2}>
              ...
              <Grid size={{ xs: 12, md: 6 }}>
                <TextField
                  ...
                  onChange={(e) => editMode && handleFieldChange('quoted_quantity', parseInt(e.target.value) || 0)}
                  disabled={!editMode}
                  size="small"
                  type="number"
                />
              ...
              <Grid size={{ xs: 12, md: 6 }}>
                <TextField
                  ...
                  onChange={(e) => editMode && handleFieldChange('units_per_box', parseInt(e.target.value) || 0)}
                  disabled={!editMode}
                  size="small"
                  type="number"
                />
              ...
              <Grid size={{ xs: 12, md: 3 }}>
                <TextField
                  ...
                  onChange={(e) => editMode && handleFieldChange('box_length', parseFloat(e.target.value) || 0)}
                  disabled={!editMode}
                  size="small"
                />
              </Grid>
              <Grid size={{ xs: 12, md: 3 }}>
                <TextField
                  ...
                  onChange={(e) => editMode && handleFieldChange('box_width', parseFloat(e.target.value) || 0)}
                  disabled={!editMode}
                  size="small"
                />
              </Grid>
              <Grid size={{ xs: 12, md: 3 }}>
                <TextField
                  ...
                  onChange={(e) => editMode && handleFieldChange('box_height', parseFloat(e.target.value) || 0)}
                  disabled={!editMode}
                  size="small"
                />
              </Grid>
              ...
              <Grid size={{ xs: 12, md: 6 }}>
                <TextField
                  ...
                  value={editMode ? (editData.shipping_method || '-') : (data.shipping_method || '-')}
                  disabled
                  size="small"
              ...
              <Grid size={12}>
                <TextField
                  ...
                  value={editMode ? editData.other_matters_cn || '' : data.other_matters_cn || ''}
                  onChange={(e) => editMode && setEditData({...editData, other_matters_cn: e.target.value})}
                  disabled={!editMode || !isChineseStaff}
                  size="small"
                />
              </Grid>
                ...
                <FormControlLabel
                  control={
                    <Checkbox
                      ...
                      onChange={(e) => editMode && setEditData({...editData, sample_available: e.target.checked})}
                      disabled={!editMode}
                    />
                  }
              ...
              <Grid size={{ xs: 12, md: 4 }}>
                <TextField
                  ...
                  value={editMode ? editData.sample_unit_price || '' : data.sample_unit_price || ''}
                  onChange={(e) => editMode && setEditData({...editData, sample_unit_price: parseFloat(e.target.value) || null})}
                  disabled={!editMode}
                  size="small"
                  helperText={editMode && editData.china_unit_price && editData.exchange_rate ? 
              ...
              <Grid size={{ xs: 12, md: 4 }}>
                <TextField
                  ...
                  value={editMode ? editData.sample_order_qty || '' : data.sample_order_qty || ''}
                  onChange={(e) => editMode && setEditData({...editData, sample_order_qty: parseInt(e.target.value)})}
                  disabled={!editMode}
                  size="small"
                />
              </Grid>
              <Grid size={{ xs: 12, md: 6 }}>
                <TextField
                  ...
                  value={editMode ? editData.sample_make_time || '' : data.sample_make_time || ''}
                  onChange={(e) => editMode && setEditData({...editData, sample_make_time: e.target.value})}
                  disabled={!editMode}
                  size="small"
                />
              </Grid>
              <Grid size={{ xs: 12, md: 6 }}>
                <TextField
                  ...
                  onChange={(e) => editMode && setEditData({...editData, sample_weight: parseFloat(e.target.value)})}
                  disabled={!editMode}
                  size="small"
                />
              </Grid>
            ...
            <Grid container spacing={2}>
              ...
              <Grid size={12}>
                <TextField
                  ...
                  onChange={(e) => editMode && setEditData({...editData, work_duration: e.target.value})}
                  disabled={!editMode}
                  size="small"
                  placeholder="예: 30일"
                />
              ...
              <Grid size={{ xs: 12, md: 4 }}>
                <TextField
                  ...
                  onChange={(e) => editMode && handleFieldChange('china_unit_price', parseFloat(e.target.value) || 0)}
                  disabled={!editMode}
                  size="small"
                  InputProps={{
              ...
              <Grid size={{ xs: 12, md: 4 }}>
                <TextField
                  ...
                  value={editMode ? editData.exchange_rate || 203.0 : data.exchange_rate || 203.0}
                  onChange={(e) => editMode && handleFieldChange('exchange_rate', parseFloat(e.target.value) || 203.0)}
                  disabled={!editMode}
                  size="small"
              ...
              <Grid size={{ xs: 12, md: 4 }}>
                <TextField
                  ...
                  value={data.exchange_rate_date ? new Date(data.exchange_rate_date).toLocaleDateString() : new Date().toLocaleDateString()}
                  disabled
                  size="small"
                />
              </Grid>
              ...
              <Grid size={{ xs: 12, md: 4 }}>
                <TextField
                  ...
                  value={editMode ? editData.china_shipping_fee || '' : data.china_shipping_fee || ''}
                  onChange={(e) => editMode && handleFieldChange('china_shipping_fee', parseFloat(e.target.value) || 0)}
                  disabled={!editMode}
                  size="small"
                  InputProps={{
                    endAdornment: <InputAdornment position="end">¥</InputAdornment>
              ...
              <Grid size={{ xs: 12, md: 4 }}>
                <TextField
                  ...
                  value={editMode ? editData.fcl_shipping_fee || '' : data.fcl_shipping_fee || ''}
                  onChange={(e) => editMode && handleFieldChange('fcl_shipping_fee', parseFloat(e.target.value) || 0)}
            ...
            <Grid container spacing={2}>
              ...
              <Grid size={{ xs: 12, md: isChineseStaff ? 6 : 6 }}>
                <TextField
                  ...
                  onChange={(e) => editMode && setEditData({...editData, export_port_cn: e.target.value})}
                  disabled={!editMode || !isChineseStaff}
                  size="small"
                />
              </Grid>
              ...
              <Grid size={12}>
                <Divider sx={{ my: 2 }}>
                  <Chip label={isChineseStaff ? '进出口/认证信息' : '수출입/인증 정보'} size="small" />
                </Divider>
              </Grid>
                        ...
                        onClick={async () => {
                          ...
                          if (koreanProductName) {
                            try {
                              ...
                              const { data: hsData, error } = await supabase.functions.invoke('hs-code-classifier', {
                                body: { 
                              ...
                              if (hsData?.results && hsData.results.length > 0) {
                                ...
                                alert(isChineseStaff ? '未找到对应的HS编码' : 'HS코드를 찾을 수 없습니다');
                              }
                            } catch (error) {
                              console.error('HS코드 조회 실패:', error);
                              alert(isChineseStaff ? 'HS编码查询失败' : 'HS코드 조회에 실패했습니다');
                            }
                          } else {
                            alert(isChineseStaff ? '请先输入产品名(韩文)' : '제품명을 먼저 입력하세요');
                          }
                        }}
                ...
                <FormControlLabel
                  control={
                    <Checkbox
                      ...
                      onChange={(e) => editMode && setEditData({...editData, certification_required: e.target.checked})}
                      disabled={!editMode}
                    />
                  }
                  ...
                  <Button
                    ...
                    onClick={async () => {
                      try {
                        ...
                        const { data: certData, error } = await supabase.functions.invoke('check-certification', {
                        ...
                        if (certData?.required !== undefined) {
                          setEditData({...editData, certification_required: certData.required});
                          alert(certData.required ? 
                            (isChineseStaff ? '需要进口认证' : '수입 인증이 필요합니다') : 
                            (isChineseStaff ? '不需要认证' : '인증이 필요하지 않습니다'));
                        }
                      } catch (error) {
              ...
              <Grid size={{ xs: 12, md: 6 }}>
                <TextField
                  ...
                  onChange={(e) => editMode && setEditData({...editData, cert_cost: parseFloat(e.target.value) || null})}
                  disabled={!editMode}
                  size="small"
                  InputProps={{
                    startAdornment: <InputAdornment position="start">₩</InputAdornment>,
              ...
              <Grid size={{ xs: 12, md: 6 }}>
                <TextField
                  ...
                  onChange={(e) => editMode && handleFieldChange('customs_rate', parseFloat(e.target.value) || 0)}
                  disabled={!editMode}
                  size="small"
                  InputProps={{
                    endAdornment: <InputAdornment position="end">%</InputAdornment>
                ...
                <Grid size={12}>
                  <Paper elevation={2} sx={{ p: 2, mt: 2, bgcolor: 'grey.50' }}>
                    ...
                    <Alert severity="success" sx={{ mb: 2 }}>
                      {isChineseStaff ? '适用税率' : '적용 관세율'}: {editData.customs_rate || data.customs_rate || 8}% 
                      {(() => {
                        const rate = editData.customs_rate || data.customs_rate;
                        if (rate === tariffDetails.korea_china_fta_rate) return ` (${isChineseStaff ? '韩中FTA' : '한중 FTA'})`;
                        if (rate === tariffDetails.fta_rate) return ` (${isChineseStaff ? 'FTA优惠' : 'FTA 특혜'})`;
                        if (rate === tariffDetails.wto_rate) return ` (${isChineseStaff ? 'WTO协定' : 'WTO 협정'})`;
                      ...
                      <Table size="small">
                        <TableHead>
                          <TableRow>
                            ...
                            <TableCell align="center">{isChineseStaff ? '税率' : '세율'}</TableCell>
                            <TableCell align="center">{isChineseStaff ? '状态' : '상태'}</TableCell>
                          </TableRow>
                        </TableHead>
                        <TableBody>
                          <TableRow>
                            ...
                            <TableCell align="center">
                              {tariffDetails.basic_rate !== undefined ? `${tariffDetails.basic_rate}%` : '8%'}
                            </TableCell>
                            <TableCell align="center">
                              {(editData.customs_rate || data.customs_rate) === tariffDetails.basic_rate && 
                                <Chip label={isChineseStaff ? '适用' : '적용'} color="success" size="small" />}
                            </TableCell>
                          </TableRow>
                          
                          {tariffDetails.wto_rate !== undefined && tariffDetails.wto_rate !== null && (
                            <TableRow>
                              <TableCell>{isChineseStaff ? 'WTO协定税率' : 'WTO 협정세율'}</TableCell>
                              <TableCell align="center">{tariffDetails.wto_rate}%</TableCell>
                              <TableCell align="center">
                                {(editData.customs_rate || data.customs_rate) === tariffDetails.wto_rate ? 
                                  <Chip label={isChineseStaff ? '适用' : '적용'} color="success" size="small" /> :
                                  <Chip label={isChineseStaff ? '未适用' : '미적용'} size="small" />}
                              </TableCell>
                            </TableRow>
                          )}
                          
                          {tariffDetails.fta_rate !== undefined && tariffDetails.fta_rate !== null && (
                            <TableRow>
                              <TableCell>{isChineseStaff ? 'FTA优惠税率' : 'FTA 특혜세율'}</TableCell>
                              <TableCell align="center">{tariffDetails.fta_rate}%</TableCell>
                              <TableCell align="center">
                                {(editData.customs_rate || data.customs_rate) === tariffDetails.fta_rate ? 
                                  <Chip label={isChineseStaff ? '适用(需C/O)' : '적용(C/O필요)'} color="success" size="small" /> :
                                  <Chip label={isChineseStaff ? 'C/O必要' : 'C/O필요'} color="warning" size="small" />}
                              </TableCell>
                            </TableRow>
                          )}
                          
                          {tariffDetails.korea_china_fta_rate !== undefined && tariffDetails.korea_china_fta_rate !== null && (
                            <TableRow>
                              <TableCell>{isChineseStaff ? '韩中FTA税率' : '한중 FTA세율'}</TableCell>
                              <TableCell align="center">{tariffDetails.korea_china_fta_rate}%</TableCell>
                              <TableCell align="center">
                                {(editData.customs_rate || data.customs_rate) === tariffDetails.korea_china_fta_rate ? 
                                  <Chip label={isChineseStaff ? '适用(需C/O)' : '적용(C/O필요)'} color="success" size="small" /> :
                                  <Chip label={isChineseStaff ? 'C/O必要' : 'C/O필요'} color="warning" size="small" />}
                              </TableCell>
                            </TableRow>
                    ...
                    {/* FTA 안내 */}
                    {(tariffDetails.fta?.rate !== undefined || tariffDetails.fcn?.rate !== undefined) && (
              ...
              <Grid size={{ xs: 12, md: 4 }}>
                <TextField
                  ...
                  label={isChineseStaff ? '样品单价' : '샘플단가'}
                  value={editMode ? editData.sample_unit_price || '' : data.sample_unit_price || ''}
                  onChange={(e) => editMode && setEditData({...editData, sample_unit_price: parseFloat(e.target.value)})}
                  disabled={!editMode}
                  size="small"
                  InputProps={{
                    endAdornment: <InputAdornment position="end">₩</InputAdornment>
              ...
              <Grid size={12}>
                <Divider sx={{ my: 2 }}>
                  <Chip label={isChineseStaff ? 'HS编码和认证' : 'HS코드 및 인증'} size="small" />
                </Divider>
              </Grid>
              <Grid size={{ xs: 12, md: 4 }}>
                <TextField
              ...
              <Grid size={{ xs: 12, md: 4 }}>
                <FormControlLabel
                  control={
                    <Checkbox
                      checked={data.certification_required || false}
                      disabled
                    />
                  }
