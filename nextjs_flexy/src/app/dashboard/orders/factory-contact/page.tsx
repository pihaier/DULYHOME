                  ...
                  <TableBody>
                    {filteredOrders.length === 0 ? (
                      ...
                      filteredOrders.map((order) => {
                        const status = getStatusLabel(order.status);
                        return (
                          <TableRow key={order.id} hover sx={{ cursor: 'pointer' }} onClick={() => handleViewOrder(order)}>
                            <TableCell>
                                <Typography variant="body2" fontWeight="bold" color="primary" noWrap>
                                {order.reservation_number}
                              </Typography>
                            </TableCell>
                              <TableCell><Typography noWrap>{order.company_name}</Typography></TableCell>
                              <TableCell><Typography noWrap>{order.factory_name}</Typography></TableCell>
                              <TableCell><Typography noWrap>{order.product_name}</Typography></TableCell>
                            <TableCell>
