...
export default function OrderDetailPage() {
  ...
  const getServiceType = (reservationNumber: string) => {
    ...
    switch(prefix) {
      ...
      case 'DL': return 'inspection';         // 검품감사는 'DL-' prefix 사용
      case 'IN': return 'inspection';         // Legacy 지원
      case 'DLSP': return 'sampling';
      case 'BO': return 'bulk_order';
      case 'DLBO': return 'bulk_order';       // Keep legacy support
  ...
  const getServiceTitle = () => {
    switch(serviceType) {
      case 'market_research': return '시장조사 상세정보';
      case 'inspection': return '검품감사 상세정보';
      case 'sampling': return '샘플링 상세정보';
      case 'bulk_order': return '대량주문 상세정보';
      default: return '주문 상세정보';
    }
  };
    ...
    try {
      ...
      const { data, error } = await supabase
        ...
        .insert({
          reservation_number: reservationNumber,
          sender_id: user?.id,
          sender_name: userProfile?.contact_person || userProfile?.company_name || user?.email || 'Unknown',
      ...
      if (error) {
        ...
        try {
          const { data: functionData, error: functionError } = await supabase.functions.invoke('translate-message', {
            body: { record: data }
          });
  ...
  if (error) {
    return (
      <PageContainer title="시장조사 상세" description="시장조사 상세 정보">
        <Breadcrumb title="시장조사 상세" items={[
          { title: '홈', to: '/' },
  ...
  // 진행 상태 단계
  const steps = [
    { label: '신청접수', icon: <BusinessIcon sx={{ fontSize: 24 }} />, date: orderData?.created_at },
    { label: '공장조사', icon: <ScienceIcon sx={{ fontSize: 24 }} />, date: orderData?.status === 'in_progress' || orderData?.status === 'completed' ? orderData?.updated_at : null },
    { label: '조사완료', icon: <CheckCircleIcon sx={{ fontSize: 24 }} />, date: orderData?.status === 'completed' ? orderData?.updated_at : null }
  ];
  const currentStep = orderData?.status === 'completed' ? 2 : orderData?.status === 'in_progress' ? 1 : 0;
    ...
    <Box sx={{ 
      ...
      <Box sx={{
                ...
                <Box>
                  ...
                  <Stack direction="row" spacing={2} alignItems="center">
                    <Typography variant="body1">
                      오더번호: {reservationNumber}
                    </Typography>
                    <Chip
                      label={orderData?.status === 'completed' ? '조사완료' : orderData?.status === 'in_progress' ? '진행중' : '신청접수'}
                      size="small"
                      sx={{ 
                        bgcolor: orderData?.status === 'completed' ? 'success.main' : orderData?.status === 'in_progress' ? 'warning.main' : 'grey.300',
                        color: 'white',
                        fontWeight: 'bold'
                      }}
                    />
        ...
        <Box sx={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
          {/* 왼쪽: 정보 */}
          <Box sx={{ flex: 1, borderRight: 1, borderColor: 'divider', overflow: 'auto', position: 'relative' }}>
            <Box sx={{ 
              borderBottom: 1, 
              borderColor: 'divider',
              position: 'sticky',
              ...
              <Tabs value={tabValue} onChange={handleTabChange} aria-label="detail tabs" variant="fullWidth">
                {serviceType === 'inspection' ? [
                  <Tab key="tab1" icon={<BusinessIcon />} label="신청정보" iconPosition="start" />,
                  <Tab key="tab2" icon={<AccessTimeIcon />} label="예약정보" iconPosition="start" />,
                  <Tab key="tab3" icon={<CheckCircleIcon />} label="검품결과" iconPosition="start" />,
                  <Tab key="tab4" icon={<AttachMoneyIcon />} label="비용정산" iconPosition="start" />
                ] : serviceType === 'sampling' ? [
                  <Tab key="tab1" icon={<BusinessIcon />} label="신청정보" iconPosition="start" />,
                  <Tab key="tab2" icon={<ShoppingCartIcon />} label="샘플정보" iconPosition="start" />,
                  <Tab key="tab3" icon={<LocalShippingIcon />} label="배송정보" iconPosition="start" />,
                  <Tab key="tab4" icon={<AttachFileIcon />} label="관련자료" iconPosition="start" />
                ] : serviceType === 'bulk_order' ? [
                  <Tab key="tab1" icon={<BusinessIcon />} label="신청정보" iconPosition="start" />,
                  <Tab key="tab2" icon={<ShoppingCartIcon />} label="주문정보" iconPosition="start" />,
                  <Tab key="tab3" icon={<LocalShippingIcon />} label="배송정보" iconPosition="start" />,
                  <Tab key="tab4" icon={<AttachFileIcon />} label="관련자료" iconPosition="start" />
                ] : [
                  <Tab key="tab1" icon={<BusinessIcon />} label="신청정보" iconPosition="start" />,
                  <Tab key="tab2" icon={<LocalShippingIcon />} label="공장정보" iconPosition="start" />,
                  <Tab key="tab3" icon={<InventoryIcon />} label="제품정보" iconPosition="start" />,
                  <Tab key="tab4" icon={<AttachMoneyIcon />} label="가격정보" iconPosition="start" />,
                  <Tab key="tab5" icon={<AttachFileIcon />} label="관련자료" iconPosition="start" />
                ]}
              </Tabs>
            </Box>
            <Box sx={{ p: 3 }}>
                      ...
                      <Table>
                        <TableBody>
                          ...
                          <TableRow>
                            ...
                            <TableCell>
                              <Chip 
                                label={data?.status === 'completed' ? '완료' : data?.status === 'in_progress' ? '진행중' : '접수'} 
                                color={data?.status === 'completed' ? 'success' : data?.status === 'in_progress' ? 'warning' : 'default'}
                                size="small"
                              />
                            </TableCell>
                          </TableRow>
                      ...
                      <Table>
                        <TableBody>
                          <TableRow>
                            ...
                            <TableCell>
                              {data?.service_subtype === 'quality_inspection' ? '품질검사' :
                               data?.service_subtype === 'factory_audit' ? '공장심사' :
                               data?.service_subtype === 'loading_inspection' ? '선적검사' : '-'}
                            </TableCell>
                          </TableRow>
                          {data?.product_name && (
                            <TableRow>
                        ...
                        <Table>
                          <TableBody>
                            ...
                            <TableRow>
                              ...
                              <TableCell>{orderData?.research_quantity?.toLocaleString() || '-'}개</TableCell>
                            </TableRow>
                          </TableBody>
                        </Table>
                        ...
                        <Table>
                          <TableBody>
                            <TableRow>
                              ...
                              <TableCell>
                                {orderData?.created_at ? new Date(orderData.created_at).toLocaleString('ko-KR') : '-'}
                              </TableCell>
                            </TableRow>
                            <TableRow>
                              <TableCell component="th" sx={{ fontWeight: 'bold', bgcolor: 'grey.50' }}>
                              ...
                              <TableCell>
                                {orderData?.detail_page ? (
                                  <a href={orderData.detail_page} target="_blank" rel="noopener noreferrer" style={{ color: '#1976d2', textDecoration: 'underline' }}>
                                    {orderData.detail_page}
                                  </a>
                                ) : '없음'}
                              </TableCell>
                            ...
                            {[orderData.logo_file]
                              .map((file: any) => (
                                <Grid size={{ xs: 12, sm: 6, md: 4 }} key={file.id}>
                                  <Paper sx={{ p: 2, textAlign: 'center', cursor: 'pointer' }} onClick={() => window.open(file.file_url, '_blank')}>
                  ...
                  <BlankCard sx={{ bgcolor: 'success.lighter' }}>
                    <CardContent>
                      ...
                      {data?.order_items && data.order_items.length > 0 ? (
                        <Table>
                          <TableBody>
                            {data.order_items.map((item: any, index: number) => (
                              <TableRow key={index}>
                                <TableCell component="th" sx={{ fontWeight: 'bold', bgcolor: 'grey.50', width: '20%' }}>
                                  아이템 {index + 1}
                                </TableCell>
                                <TableCell>
                                  <Stack spacing={1}>
                                    ...
                                    <Typography variant="body2">
                                      <strong>주문 수량:</strong> {item.order_quantity?.toLocaleString() || '-'}개
                                    </Typography>
                                    <Typography variant="body2">
                                      <strong>단가:</strong> ¥{item.unit_price?.toLocaleString() || '-'}
                                    </Typography>
                                    <Typography variant="body2">
                                      <strong>총액:</strong> ¥{item.total_price?.toLocaleString() || '-'}
                                    </Typography>
                                    {item.specifications && (
                                      <Typography variant="body2">
                        ...
                        <Typography color="text.secondary">등록된 주문 아이템이 없습니다.</Typography>
                      )}
                    </CardContent>
                  </BlankCard>
                            ...
                            <TableCell>
                              <Chip 
                                label={
                                  ...
                                  data?.status === 'quoted' ? '견적 완료' : '접수'
                                } 
                                color={
                                  data?.status === 'completed' ? 'success' :
                                  data?.status === 'in_progress' ? 'info' :
                                  data?.status === 'quoted' ? 'warning' : 'default'
                                }
                                size="small"
                              />
                            </TableCell>
                ...
                <Stack spacing={3} sx={{ p: 2 }}>
                  <BlankCard sx={{ bgcolor: 'success.lighter' }}>
                    <CardContent>
                      ...
                      {data?.sampleOrder?.sample_items && data.sampleOrder.sample_items.length > 0 ? (
                        <Table>
                          <TableBody>
                        ...
                        <Typography color="text.secondary">등록된 샘플 아이템이 없습니다.</Typography>
                      )}
                    </CardContent>
                  </BlankCard>
                        ...
                        <TableBody>
                          <TableRow>
                            ...
                            <TableCell>
                              <Chip 
                                label={
                                  data?.sampleOrder?.sample_status === 'shipped' ? '배송완료' :
                                  data?.sampleOrder?.sample_status === 'in_transit' ? '배송중' :
                                  data?.sampleOrder?.sample_status === 'preparing' ? '준비중' : '대기중'
                                } 
                                color={
                                  data?.sampleOrder?.sample_status === 'shipped' ? 'success' :
                                  data?.sampleOrder?.sample_status === 'in_transit' ? 'info' :
                                  data?.sampleOrder?.sample_status === 'preparing' ? 'warning' : 'default'
                                }
                                size="small"
                              />
                            </TableCell>
                      ...
                      <Table>
                        <TableBody>
                          <TableRow>
                            ...
                            <TableCell>
                              <Chip 
                                label={data?.schedule_type === 'already_booked' ? '이미 예약' : '두리무역 협의'} 
                                color={data?.schedule_type === 'already_booked' ? 'success' : 'default'}
                                size="small"
                              />
                            </TableCell>
                          </TableRow>
            ...
            <TabPanel value={tabValue} index={2}>
              {serviceType === 'bulk_order' ? (
                      ...
                      <Table>
                        <TableBody>
                          ...
                          <TableRow>
                            ...
                            <TableCell>
                              {data?.delivery_date ? new Date(data.delivery_date).toLocaleDateString('ko-KR') : '-'}
                            </TableCell>
                          </TableRow>
                          <TableRow>
                            <TableCell component="th" sx={{ fontWeight: 'bold', bgcolor: 'grey.50' }}>
                            ...
                            <TableCell>
                              <Chip 
                                ...
                                color={
                                  data?.delivery_method === 'DDP' ? 'primary' : 
                                  data?.delivery_method === 'FOB' ? 'secondary' : 'default'
                                }
                                size="small"
                              />
                            </TableCell>
                  ...
                  <BlankCard sx={{ bgcolor: 'info.lighter' }}>
                    <CardContent>
                      ...
                      <Typography variant="body2" color="text.secondary">
                        {data?.status === 'completed' ? '대량주문이 완료되었습니다.' :
                         data?.status === 'in_progress' ? '대량주문이 진행 중입니다.' :
                         data?.status === 'quoted' ? '견적이 완료되었습니다. 결제를 진행해주세요.' :
                      ...
                      <Table>
                        <TableBody>
                          <TableRow>
                            ...
                            <TableCell>{data?.sampleOrder?.sample_receive_address || '-'}</TableCell>
                          </TableRow>
                          <TableRow>
                            <TableCell component="th" sx={{ fontWeight: 'bold', bgcolor: 'grey.50' }}>
                              수령인
                          ...
                          <TableRow>
                            ...
                            <TableCell>
                              <Chip 
                                ...
                                color={data?.sampleOrder?.shipping_method === '항공' ? 'primary' : 'secondary'}
                                size="small"
                              />
                            </TableCell>
                          </TableRow>
                      ...
                      <Table>
                        <TableBody>
                          <TableRow>
                            ...
                            <TableCell>
                              {data?.sampleOrder?.customs_clearance_type === 'personal' ? '개인통관' : '일반통관'}
                            </TableCell>
                          </TableRow>
                          <TableRow>
                            <TableCell component="th" sx={{ fontWeight: 'bold', bgcolor: 'grey.50' }}>
                            ...
                            <TableCell>{data?.sampleOrder?.customs_clearance_number || '-'}</TableCell>
                          </TableRow>
                        </TableBody>
                      </Table>
                    </CardContent>
                ...
                <Stack spacing={3} sx={{ p: 2 }}>
                  {data?.inspection_report ? (
                    <>
                      <BlankCard sx={{ bgcolor: 'success.lighter' }}>
                        <CardContent>
                          <Typography variant="h6" gutterBottom fontWeight="bold">검품 보고서</Typography>
                          <Button
                            variant="contained"
                      ...
                      {data?.pass_fail_status && (
                        <BlankCard sx={{ bgcolor: 'info.lighter' }}>
                          <CardContent>
                            <Typography variant="h6" gutterBottom fontWeight="bold">검품 결과</Typography>
                            <Chip 
                      ...
                      {data?.improvement_items && data.improvement_items.length > 0 && (
                        <BlankCard sx={{ bgcolor: 'warning.lighter' }}>
                          <CardContent>
                            <Typography variant="h6" gutterBottom fontWeight="bold">개선 요구사항</Typography>
                ...
                {/* 제품 사진 */}
                {(() => {
                  const productImages = files?.filter((file: any) => 
                      ...
                      <TableBody>
                        ...
                        {(orderData?.logo_required || orderData?.custom_box_required) && (
                          <TableRow>
                            ...
                            <TableCell>
                              {orderData?.custom_box_required ? (orderData?.box_details || '-') : '-'}
                            </TableCell>
                          </TableRow>
                        )}
                      </TableBody>
                ...
                <Stack spacing={3} sx={{ p: 2 }}>
                  {data?.total_cost ? (
                    <>
                      <BlankCard sx={{ bgcolor: 'info.lighter' }}>
                        <CardContent>
                          <Typography variant="h6" gutterBottom fontWeight="bold">결제 정보</Typography>
                          <Table size="small">
                            <TableBody>
                              <TableRow>
                      ...
                      <BlankCard sx={{ bgcolor: 'primary.lighter' }}>
                        <CardContent>
                          <Typography variant="h6" gutterBottom fontWeight="bold">문서 다운로드</Typography>
                          <Stack spacing={1}>
                            {data.quotation_pdf && (
                              <Button
                                variant="outlined"
                      ...
                      <Table>
                        <TableBody>
                          <TableRow>
                            ...
                            <TableCell sx={{ width: '35%' }}>{data?.unit_price?.toLocaleString() || '조사중'}</TableCell>
                            <TableCell component="th" sx={{ fontWeight: 'bold', width: '15%', bgcolor: 'grey.50' }}>
                              1차 결제금액
                            </TableCell>
                            <TableCell sx={{ width: '35%' }}>{data?.first_payment?.toLocaleString() || '조사중'}</TableCell>
                          </TableRow>
                          <TableRow>
                            <TableCell component="th" sx={{ fontWeight: 'bold', bgcolor: 'grey.50' }}>
                              2차 결제 예상 금액
                            </TableCell>
                            <TableCell>{data?.second_payment_estimate?.toLocaleString() || '조사중'}</TableCell>
                            <TableCell component="th" sx={{ fontWeight: 'bold', bgcolor: 'grey.50' }}>
                              1차 상세비용
                            </TableCell>
                            <TableCell>{data?.first_payment_detail || '조사중'}</TableCell>
                          ...
                          <TableRow>
                            ...
                            <TableCell>{((data?.exw_total || 0) * (data?.tariff || 8) / 100).toLocaleString() || '조사중'}</TableCell>
                            <TableCell component="th" sx={{ fontWeight: 'bold', bgcolor: 'grey.50' }}>
                              VAT
                            </TableCell>
                          ...
                          <TableRow>
                            ...
                            <TableCell>{data?.total_supply_price?.toLocaleString() || '조사중'}</TableCell>
                            <TableCell component="th" sx={{ fontWeight: 'bold', bgcolor: 'grey.50' }}>
                  ...
                  {false && (
                    <BlankCard sx={{ bgcolor: 'success.lighter' }}>
                      <CardContent>
                        ...
                        <Grid container spacing={2}>
                          {files
                            .filter((f: any) => f.upload_category === 'product' && f.mime_type?.startsWith('image/'))
                            .map((file: any) => (
                              <Grid size={{ xs: 12, sm: 6, md: 4, lg: 3 }} key={file.id}>
                                <Box
                                  sx={{
                    ...
                    <Stack spacing={2}>
                      {chatMessages.map((msg: any) => (
                        <Box
                          ...
                          sx={{
                            display: 'flex',
                            justifyContent: msg.sender_id === data?.user_id ? 'flex-end' : 'flex-start'
                          }}
                        >
                          <Paper
                            sx={{
                              p: 1.5,
                              maxWidth: '70%',
                              bgcolor: msg.sender_id === data?.user_id ? 'primary.main' : 'grey.100',
                          ...
                          >
                            ...
                            {msg.translated_message && (
                              <Typography variant="body2" sx={{ mt: 0.5, fontStyle: 'italic', opacity: 0.8 }}>
                                {msg.translated_message}
                              </Typography>
                            )}
