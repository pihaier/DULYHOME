  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const allFiles = [
          ...(data.application_photos || []).map((f: any) => ({
            ...f,
            file_type: 'product',
            file_url: f.url,
          })),
          ...(data.logo_files || []).map((f: any) => ({
            ...f,
            file_type: 'logo',
            file_url: f.url,
          })),
          ...(data.box_files || []).map((f: any) => ({
            ...f,
            file_type: 'box_design',
            file_url: f.url,
          })),
        ];
        setFiles(allFiles);
                    ...
                    <CardContent>
                      <Typography variant="h6" fontWeight="bold" gutterBottom sx={{ mb: 2 }}>
                        박스 디자인 파일 (
                        {
                          files.filter(
                            (f) =>
                              f.file_type === 'box_design' || f.upload_category === 'box_design'
                          ).length
                        }
                        개)
                      </Typography>
