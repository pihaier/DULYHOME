...
const CONTAINERS = {
  ...
  '40hc': { length: 12.03, width: 2.35, height: 2.7, cbm: 76.4 },
};
      ...
      <CardContent>
        <Stack spacing={2}>
          ...
          <Stack direction="row" spacing={2}>
            ...
            <Chip
              label={`공간 활용률 ${efficiency.toFixed(1)}%`}
          ...
          {boxPositions.length < quantity && (
            <Alert severity="warning">
              {containerType} 컨테이너에는 {boxPositions.length}개만 적재 가능합니다. 나머지{' '}
              {quantity - boxPositions.length}개는 추가 컨테이너가 필요합니다.
            </Alert>
          )}
          ...
          >
            <Canvas camera={{ position: [15, 10, 15], fov: 50 }}>
              ...
              <lineSegments>
                <edgesGeometry
                  args={[
                    new THREE.BoxGeometry(container.length, container.height, container.width),
                  ]}
                />
