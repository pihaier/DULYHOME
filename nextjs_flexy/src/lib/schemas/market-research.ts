...
export const marketResearchSchema = z
  .object({
    ...
    photos: z
      ...
      .refine(
        (files) =>
          files.every((file) => ['image/jpeg', 'image/png', 'image/gif'].includes(file.type)),
        '이미지 파일만 업로드 가능합니다 (JPG, PNG, GIF)'
      ),

    // 5. 상세페이지 URL (선택)
    detailPage: z.string().url('올바른 URL을 입력해주세요').optional().or(z.literal('')),

    // 6. 로고 인쇄 관련 (조건부)
    logoRequired: z.boolean(),
    logoFile: z.array(z.instanceof(File)).max(5, '로고 파일은 최대 5개까지 업로드 가능합니다'),
    logoPrintDetails: z
      .string()
  ...
  .refine(
    (data) => {
      ...
      if (data.boxDesignFile && data.boxDesignFile.length > 0) {
        const validTypes = ['image/jpeg', 'image/png', 'application/pdf'];
        const invalidBoxFiles = data.boxDesignFile.filter(
